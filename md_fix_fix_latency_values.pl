...
my $bst_ask_quant_fix : shared = 0;
my $bst_ask_quant_fix1 : shared = 0;

my $bst_bid_quant_fix : shared = 0;
my $bst_bid_quant_fix1 : shared = 0;
...
my @queue = ([(0) x 4],[(0) x 4]);
my @queue1 = ([(0) x 4],[(0) x 4]);
...
if ($str_fix)	   
{
 ...
 #my @tags = split(/\|/,$str_fix);
    ...
    for(my $i=0;$i<$tags_count;$i++) {
	  ...
	  #bid quant---------vvv
	  if (($tags[$i]=~$r271) and ($bid_quant_flag==0))
	  {
       my ($key, $value) = split('=',$tags[$i]);
	   $bst_bid_quant_fix = $value;
	   $bid_quant_flag=1;
	   next;
	  }
	  #bid quant---------^^^
	  ...
	  #ask quant---------vvv
	  if (($tags[$i]=~$r271) and ($bid_quant_flag==1))
	  {
	   my ($key, $value) = split('=',$tags[$i]);
	   $bst_ask_quant_fix = $value;
	   next;
	  }
      #ask quant---------^^^
...
my $bst_ask_fix_tmp =  my $bst_bid_fix_tmp = my $bst_ask_fix_tmp1 = my $bst_bid_fix_tmp1 = my $bst_ask_quant_fix_tmp = my $bst_ask_quant_fix_tmp1 = 0;
...
my ($seconds) = time;
my $seconds_3f = sprintf("%.6f", $seconds);



#print "epoch: $seconds_3f best_ask fix/fix1: $bst_ask_fix $bst_ask_fix1  best_bid fix/fix1: $bst_bid_fix $bst_bid_fix1 ask quant fix/fix1: $bst_ask_quant_fix $bst_ask_quant_fix1 bid quant fix/fix1: $bst_bid_quant_fix $bst_bid_quant_fix1\n";

#----Grouping bst ask and best bid in two separate arrays--------------------------vvv
#if (($bst_ask_fix_tmp != $bst_ask_fix) or  ($bst_bid_fix_tmp != $bst_bid_fix))
if ($bst_ask_fix_tmp != $bst_ask_fix)
{ 
shift(@queue);
push (@queue, [$seconds_3f, $bst_ask_fix, $bst_bid_fix,$bst_ask_quant_fix]);
#print "$queue[0][0] $queue[0][1] $queue1[0][1]\n";

#print "epoch/epoch1:  $queue[0][0] $queue1[0][0] bst_ask/bst_ask1: $queue[0][1] $queue1[0][1]\n";
#print "epoch time/epoch time1:  $queue[1][0] $queue1[1][0] bst_ask/bst_ask1: $queue[1][1] $queue1[1][1]\n";
#print "$seconds_3f, $bst_ask_fix, $bst_bid_fix\n";
}

if ($bst_ask_fix_tmp1 != $bst_ask_fix1)
{
shift(@queue1);
push (@queue1, [$seconds_3f, $bst_ask_fix1, $bst_bid_fix1,$bst_ask_quant_fix1]);
#print "$queue[0][0] $queue[0][1] $queue1[0][1]\n";

#print "epoch/epoch1:  $queue[0][0] $queue1[0][0] bst_ask/bst_ask1: $queue[0][1] $queue1[0][1]\n";
#print "epoch time/epoch time1:  $queue[1][0] $queue1[1][0] bst_ask/bst_ask1: $queue[1][1] $queue1[1][1]\n";
#print "$seconds_3f, $bst_ask_fix1, $bst_bid_fix1\n";
}


#if (($bst_ask_fix_tmp != $bst_ask_fix)  or ($bst_ask_fix_tmp1 != $bst_ask_fix1))
#{
#unless ($queue[0][1] eq $queue1[0][1]) {
# if ($queue1[0][1] eq $queue[1][1])
# {
# my $diff = $queue1[0][0] - $queue[1][0];
# print "              Delay: $diff \n ";
# }
#}
#}
...
$bst_ask_fix_tmp = $bst_ask_fix;
$bst_bid_fix_tmp = $bst_bid_fix;

$bst_ask_fix_tmp1 = $bst_ask_fix1;
$bst_bid_fix_tmp1 = $bst_bid_fix1;

$bst_ask_quant_fix_tmp = $bst_ask_quant_fix;
$bst_ask_quant_fix_tmp1 = $bst_ask_quant_fix1;

#----Grouping bst ask and best bid in two separate arrays--------------------------^^^


#epoch time: 1366103655.685413 bst_ask/bst_ask1: 125.98 125.98
#epoch time: 1366103655.685413 bst_ask/bst_ask1: 125.98 125.98
#epoch time: 1366103655.685413 bst_ask/bst_ask1: 125.98 125.98
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.98
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.98
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.98
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.94
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.94
#epoch time: 1366103666.286034 bst_ask/bst_ask1: 125.94 125.94
